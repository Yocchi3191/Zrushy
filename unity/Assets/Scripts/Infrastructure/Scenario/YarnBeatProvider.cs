...
namespace Zrushy.Unity.Infrastructure.Scenario
{
	/// <summary>
	...
	public class YarnBeatProvider : IBeatProvider
	{
		...
		public event Action<Beat>? OnBeatReady;
		...
		public YarnBeatProvider(DialogueRunner dialogueRunner, ZrushyDialoguePresenter presenter)
		{
			...
			dialoguePresenter.OnLineReady += line =>
			{
				Current = ConvertToBeat(line);
				OnBeatReady?.Invoke(Current);
			};
			dialoguePresenter.OnDialogueCompleted += () =>
			{
				IsCompleted = true;
				OnCompleted?.Invoke();
			};
